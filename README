...
error code to the callee (if DB_ERRORMODE is set to
db_MySQLi::DBOF_RETURN_ALL_ERRORS) or prints out an error message and
...
See examples/test_desc_tables.php for an example how to use this method.
...
TRUE indicates that Autocommit is enabled.
...
is useful if you want to use the mysqli_* functions of PHP on an already
...
TRUE if persistant connections are enabled, else false.
...
See examples/test_locale.php for an example.
...
-----------------------------------------------------------------------------
mixed Query (string $querystring, [integer $resflag = MYSQLI_ASSOC],
             integer $no_exit)
-----------------------------------------------------------------------------
...
A more detailed description how to use compatibility mode is described below.
...
- db_MySQLi::DBOF_DEBUG_SCREEN  = Turn on debugging on screen
...
- db_MySQLi::DBOF_RETURN_ALL_ERRORS = No error/autoexit, just return the
                                      mysqli_error code.
...
See examples/test_desc_table.php for an example.
...
either set the configuration define "MYSQLDB_COMPATIBLE_MODE" to TRUE or
